...
function generateStars(){
  ...
  for(let i=0;i<150;i++){
    ...
    const durM=Math.random()*30+20;
    const delayM=-(Math.random()*durM);
    const durT=Math.random()*10+5;
    const delayT=-(Math.random()*durT);
    s.style.cssText=`
      left:${x}vw;top:${y}vh;width:${size}px;height:${size}px;
      --dx:${dx}vw;--dy:${dy}vh;
      --movement-duration:${durM}s;--movement-delay:${delayM.toFixed(2)}s;
      --twinkle-duration:${durT.toFixed(2)}s;--twinkle-delay:${delayT.toFixed(2)}s;`;
