...
function generateAnimatedStars() {
    ...
    for (let i = 0; i < starCount; i++) {
        ...
        const delayMovement = -(Math.random() * durationMovement); // atraso negativo → já em movimento

        // Parâmetros de Brilho
        const TWINKLE_DURATION = 60; 
        const durationTwinkle = TWINKLE_DURATION;
        // atraso totalmente aleatório e negativo → estrela já entra visível
        const delayTwinkle = -(Math.random() * durationTwinkle);

        // Injetando como variáveis CSS
        star.style.cssText = `
            left: ${x}vw; 
            top: ${y}vh;
            width: ${size}px;
            ...
            --twinkle-duration: ${durationTwinkle}s;
...
// -----------------------------------------------------
// LOGO DE DESBLOQUEIO E INICIALIZAÇÃO
// -----------------------------------------------------
        ...
        if (password === CORRECT_PASSWORD) {
            ...
            document.body.innerHTML = `
                <div class="main-container">
                    <div class="block-card" style="padding: 4rem;">
                        <h2 class="text-3xl font-bold text-green-500">Acesso Liberado!</h2>
                        <p class="text-gray-400 mt-2">Redirecionando para o seu destino...</p>
                    </div>
                </div>`;
